...
export async function renderCourseDetail() {
    ...
    async function refreshProgress() {
        ...
        if (progText) progText.textContent = state.user
            ? `Fortschritt: ${pct}% (${done.length}/${allLessons.length} Lektionen)`
            : `Fortschritt: 0% (Bitte einloggen, um zu speichern)`;
    ...
    `).join("");
        ...
        qsa("[data-lesson]").forEach(btn => {
            btn.onclick = async () => {
                if (!state.user) {
                    alert("Bitte einloggen, um deinen Fortschritt zu speichern.");
                    return;
                }
...
export async function renderProfile() {
    ...
    async function refresh() {
        if (!state.user) {
            ...
            if (stateEl) stateEl.textContent = "Bitte einloggen, um Fortschritt & Lernblätter zu speichern.";
            return;
        }
        if (nameEl) nameEl.textContent = state.user.displayName || "User";
        if (emailEl) emailEl.textContent = state.user.email || "-";
        if (stateEl) stateEl.textContent = "Dein Fortschritt wird in deinem Account gespeichert (Firestore).";
    }
